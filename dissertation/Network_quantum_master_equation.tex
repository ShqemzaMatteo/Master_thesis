...
Nel 2 capitolo abbiamo introdotto la density matrix per i network, basata sulla communicability matrix. Questa grandezza esprime il modo in cui l'informazione si diffonde all'interno di un network a cui è applicata una dinamica di random walk. 
In questo capitolo colleghiamo la suddetta ad un network con una dinamica di quantum random walk che subisce anche delle fluttuazioni termiche. Studiando il sistema tramite una Lindblad master equation, si ricava la sua stazionary distribution che coincide proprio con la density matrix per i networks.  

In chapter \ref{C_Density_Matrix} we have introduced the density matrix for a network based on the dynamical communicability matrix. This quantity express the possible way the 
...
With  Von Neumann entropy we can study the relaxation.
\begin{comment} 
We know some complex dynamics, for example, the control operator for the SIS epidemiological model is
\begin{equation}
    \frac{d}{dt}x_i = -\sum_j \left(B\delta_{ij} + R(1-x_i)\right)x_j
\end{equation}

And for the Kuramoto's synchronization model \cite{Kuramoto}
\begin{equation}
\frac{d}{dt} x_i =\omega_i  + R \sum_j A_{ij}\sin(x_j-x_i).
\end{equation}
\end{comment}
